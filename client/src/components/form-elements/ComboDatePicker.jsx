...
export class ComboDatePicker extends React.Component {
    ...
    constructor(props) {
        ...
        if (props.placeholder !== undefined && props.placeholder !== null && (typeof props.placeholder === 'string' || Array.isArray(props.placeholder))) {
            let holders = typeof props.placeholder === 'string' ? props.placeholder.split(',') : props.placeholder;
            if (holders.length === 3) {
        ...
        this.yearList = [];
        for (let i = this.maxDate.getFullYear(); i >= this.minDate.getFullYear(); i--) {
    ...
    render() {
        ...
        let selects = {
            'd': (<ComboDatePicker.MySelect attrs={this.attrsDate}
                                            type="date" model={this.model} items={dateList}
            ...
            'm': (<ComboDatePicker.MySelect attrs={this.attrsMonth}
                                            type="month" model={this.model} items={monthList}
            ...
            'y': (<ComboDatePicker.MySelect attrs={this.attrsYear}
                                            type="year" model={this.model} items={this.yearList}
    ...
    getDateList() {
        ...
        let end = this.model !== null ? ComboDatePicker.maxDate(this.model.getMonth() + 1, this.model.getFullYear()) : 31;
    ...
    getMonthList() {
        ...
        let start = this.model !== null && this.model.getFullYear() === this.minDate.getFullYear() ? this.minDate.getMonth() : 0;
        let end = this.model !== null && this.model.getFullYear() === this.maxDate.getFullYear() ? this.maxDate.getMonth() : 11;
    ...
    static adjustTimezone(myDate, myTimezone) {
        let offset = isNaN(myTimezone) ? new Date().getTimezoneOffset() : parseFloat(myTimezone) * 60;
...
ComboDatePicker.MySelect = class extends React.Component {
    ...
    render() {
        ...
        if (this.placeholder) {
            ...
            if(!this.value) { options.unshift(<option value="" key=""> </option>); }
        }

        // Return value.
        return <select onChange={this.handleChange} {...this.attributes} value={this.value}>{options}</select>;
    }
